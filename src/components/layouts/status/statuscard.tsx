...
export default function StatusCard({ title, value, icon, status = "active", desc, variant = "default" }: StatusCardProps) {
  const colors = {
  ...
  if (variant === "compact") {
    return (
      <div className={`p-4 rounded-xl border ${colors[status]} flex items-center gap-4`}>
        <div className="p-2 rounded-lg bg-black/20 text-current">
          {icon}
        </div>
        <div>
  ...
  return (
    <div className={`relative overflow-hidden p-6 rounded-2xl border ${colors[status]} transition-all hover:scale-[1.02]`}>
      <div className="flex justify-between items-start mb-4">
        ...
        <div className={`p-3 rounded-xl bg-black/30 ${colors[status]}`}>
          {icon}
        </div>
      </div>
